...
async def get_ai_ready():
    ...
    Si no lo está, intenta auto-inicializar y detectar los modelos antes de fallar.
    """
    if not AI_STATE.get("ready"):
        try:
            await initialize_real_models()
        except Exception as e:
            logger.error(f"Error reintentando inicializar IA: {e}")
            
    if not AI_STATE.get("ready"):
        raise HTTPException(
            status_code=503, 
            detail=f"Servicio de IA no disponible: {AI_STATE.get('message', 'Iniciando servicio de IA...')}"
        )
    return AI_STATE
# ------------------------------------------
...
async def initialize_real_models():
    ...
    try:
        ...
        model_names = [m.get('name', '') for m in real_models]
        
        # Filtramos para no seleccionar por defecto modelos gigantes (27B, 35B, 70B) que saturan CPU
        light_models = [m for m in model_names if not any(heavy in m.lower() for heavy in ['27b', '35b', '70b'])]
        candidate_pool = light_models if light_models else model_names

        # Preferencias ordenadas para predicción (modelos ágiles y de buen razonamiento)
        pred_preferences = ['qwen3.5:9b', 'qwen3:4b', 'qwen3.5:2b', 'gemma4:12b', 'ornith:9b', 'qwen2.5', 'qwen3', 'deepseek', 'llama']
        chat_preferences = ['gemma4:12b', 'ornith:9b', 'qwen3.5:9b', 'qwen3:4b', 'qwen3.5:2b', 'qwen', 'llama']
        
        prediction_model = None
        for pref in pred_preferences:
            for name in candidate_pool:
                if pref.lower() in name.lower():
                    prediction_model = name
                    break
            if prediction_model:
                break
                
        chat_model = None
        for pref in chat_preferences:
            for name in candidate_pool:
                if pref.lower() in name.lower():
                    chat_model = name
                    break
        ...
        if not prediction_model:
            prediction_model = candidate_pool[0]
        if not chat_model:
            chat_model = candidate_pool[0]

        AI_CONFIG_CACHE["default_prediction_model"] = prediction_model
        AI_CONFIG_CACHE["default_chat_model"] = chat_model
        
        # --- Actualizamos el "semáforo" para ponerlo en VERDE ---
        AI_STATE["models"]["prediction"] = prediction_model
        AI_STATE["models"]["chat"] = chat_model
        AI_STATE["ready"] = True
...
async def get_completely_real_predictions(
    ...
    try:
        ...
        base_query = db.query(MachinePrediction).options(
        ...
        if machine_id:
            base_query = base_query.filter(MachinePrediction.machine_id == machine_id)
        
        # Filtrar por sección si es jefe de sección
        if current_user.role.nombre == "Jefe de Sección":
            base_query = base_query.join(Machine).filter(Machine.section_id == current_user.section_id)
        
        recent_query = base_query.filter(MachinePrediction.created_at >= datetime.utcnow() - timedelta(days=days))
        real_predictions = recent_query.order_by(desc(MachinePrediction.created_at)).all()
        
        if not real_predictions:
            real_predictions = base_query.order_by(desc(MachinePrediction.created_at)).all()
...
@router.post("/initialize-real-models")
...
async def predict_machine_maintenance(
    ...
    try:
        ...
        historical_orders = db.query(WorkOrder).filter(
            ...
            WorkOrder.created_at >= datetime.utcnow() - timedelta(days=3650)
        ).order_by(desc(WorkOrder.created_at)).limit(50).all()
        
        if len(historical_orders) < 1:
            return {
                "success": False,
                "message": f"Máquina {machine.nombre} tiene insuficientes datos históricos ({len(historical_orders)} órdenes)",
