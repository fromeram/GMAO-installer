# src/routes.py (Completo y Corregido v12 - SIN NINGÚN UserModel)
import json
import logging
...
from fastapi import Request, APIRouter, HTTPException, Depends, File, UploadFile, Form, BackgroundTasks, Body, status, Query, Response
...
from src.models.audit_response import AuditLogResponse
...
# <<<--- DEFINICIÓN DE ROLES DE GESTIÓN CORREGIDA --- >>>
...
# <<<--- ROLES CON PERMISOS ESPECÍFICOS CORREGIDOS --- >>>
# Roles que pueden ver inventario, reportes y comparar precios (NO incluye Mecánicos)
INVENTORY_ACCESS_ROLES = ["Administrador", "Jefe de Mantenimiento", "Jefe Sección", "Calidad", "Contabilidad"]

# Roles que pueden ver precios (incluye información financiera)
FINANCIAL_ACCESS_ROLES = ["Administrador", "Jefe de Mantenimiento", "Calidad", "Contabilidad"]

# Roles que pueden crear/editar productos (no incluye Calidad/Contabilidad - solo consulta)
PRODUCT_EDIT_ROLES = ["Administrador", "Jefe de Mantenimiento"]

# Roles básicos de trabajo que SOLO ven órdenes asignadas a ellos
RESTRICTED_WORKER_ROLES = ["Mecánico"]

# Roles de consulta que pueden ver TODAS las órdenes pero no gestionarlas
CONSULTANT_ROLES = ["Calidad", "Contabilidad"]

# Roles que pueden ver calendarios (TODOS los roles trabajadores + gestión)
CALENDAR_ACCESS_ROLES = ["Administrador", "Jefe de Mantenimiento", "Jefe Sección", "Mecánico", "Calidad", "Contabilidad"]

# <<<--- FIN DEFINICIÓN --- >>>

# ===============================================================================
# FUNCIONES DE DEPENDENCIA ACTUALIZADAS
# ===============================================================================
...
class UserUpdate(BaseModel):
    # Campos que se pueden actualizar (NO incluimos password aquí)
    username: Optional[str] = None
    role_id: Optional[int] = None
    section_id: Optional[int] = None # Permitir asignar null para quitar sección
...
class InventoryReadBasic(BaseModel):
     id: int
     product_name: str
     quantity: Optional[int] = None  # ← Hacer opcional
     class Config: orm_mode = True
class CodeBase(BaseModel):
    code: str = Field(..., max_length=50); description: str = Field(..., max_length=255)
class CodeRead(CodeBase):
    id: int
    active: Optional[bool] = True  # ← Opcional con valor por defecto 
    class Config: orm_mode = True
...
class CodeCreate(CodeBase): pass; active: bool = True
class CodeUpdate(BaseModel): code: Optional[str]=Field(None,max_length=50); description: Optional[str]=Field(None,max_length=255); active: Optional[bool] = None
...
class WorkOrderBase(BaseModel):
    ...
    operator: str
    assigned_to_id: Optional[int] = None  # ⭐ CAMBIAR A OPCIONAL
    repuesto_id: Optional[int] = None
    quantity_used: Optional[int] = Field(default=0, ge=0)
...
class WorkOrderCreate(WorkOrderBase):
    ...
    # NUEVO: Campo para checklist integrado
...
class WorkOrderCompleteUpdate(BaseModel):
    ...
    # NUEVOS: Campos para checklist integrado
    task_list_id: Optional[int] = None
    checklist_data: Optional[Dict[str, Any]] = None
class WorkOrderTechnicianBase(BaseModel):
    user_id: int
    role: str = Field("apoyo", regex="^(principal|apoyo|supervisor)$")
...
class WorkOrderTechnicianUpdate(BaseModel):
    role: Optional[str] = Field(None, regex="^(principal|apoyo|supervisor)$")
...
class WorkOrderRead(BaseModel):
    ...
    title: str
    details: Optional[str] = None
    work_type: str  # ✅ Sin restricción aquí, se valida en create
    section_id: int
    line_id: Optional[int] = None  # ✅ Permite None
    machine_id: Optional[int] = None  # ✅ Puede ser None
    operator: str
    assigned_to_id: int
    status: str
    ...
    # ✅ OBJETOS ANIDADOS (se populan desde SQLAlchemy)
    ...
    repuesto: Optional[InventoryReadBasic] = None
...
# 5. Función auxiliar para manejar el checklist (añadir ANTES de los endpoints)
...
# ✅ MODELO PARA ACTUALIZAR ÓRDENES DE CAMBIO DE FORMATO
...
class InventoryReadBasic(BaseModel):
    ...
    product_name: str
    quantity: int
    tipo: Optional[str] = "mecánico"  # Nuevo campo
...
class InventarioBase(BaseModel):
    ...
    tipo: Optional[str] = Field(default="mecánico")  # Nuevo campo con validación
    
    class Config:
        orm_mode = True

class InventarioCreate(InventarioBase):
    # Validación adicional para el tipo
    tipo: str = Field(default="mecánico", regex="^(mecánico|eléctrico|neumático|limpieza)$")
class InventarioUpdate(BaseModel):
    ...
    tipo: Optional[str] = Field(default=None, regex="^(mecánico|eléctrico|neumático|limpieza)$")  # INCLUIR LIMPIEZA    
    class Config:
        orm_mode = True
...
class SupplierPriceBase(BaseModel):
    ...
    def validate_discount(cls, v):
        if v is None:
            return Decimal('0.0') # Default si es None
        try:
            # Intentar conversión asegurando que v_dec se defina
            v_dec = Decimal(str(v))
        except Exception:
            # Si la conversión falla, lanzar error claro
            raise ValueError(f"Valor inválido para descuento: '{v}'")

        # Ahora v_dec existe, comprobar rango
    ...
    def validate_price(cls, v):
        if v is None:
            # El precio no debería ser null, lanzar error
...
# 13. Ausencias (Tus modelos existentes)
# =====================================
...
class AbsenceRead(AbsenceBase):
    id: int; created_at: date # Asegúrate que el campo se llama así en el modelo SQLAlchemy
    # user: Optional[UserReadBasic] = None # Descomenta si necesitas info de usuario aquí
    class Config: orm_mode = True # orm_mode es para Pydantic V1, usa from_attributes=True para V2+
...
# 15. Overrides de Turno (v2 - Usando shift_code)
# ==============================================
class ShiftOverrideBase(BaseModel):
    user_id: int
    date: date
    actual_shift_code: str = Field(..., max_length=10) # Ej: M, T, N
    notes: Optional[str] = None

class ShiftOverrideCreate(ShiftOverrideBase):
    @validator('actual_shift_code')
    def validate_shift_code(cls, v):
        valid_codes = ['M', 'T', 'N'] # Ajusta si necesitas otros códigos
        if v not in valid_codes:
            raise ValueError(f"Código de turno inválido. Usar: {', '.join(valid_codes)}")
        return v

# Necesitas UserReadBasic y ShiftPatternReadBasic definidos ANTES en este archivo
...
# 16. Solicitudes de Vacaciones (NUEVO)
# ====================================
# Asegúrate que UserReadBasic está definido antes
...
class VacationRequestUpdate(BaseModel): # Para aprobar/rechazar
...
# Modelo para la respuesta del NUEVO endpoint /vacation-requests/calendar
class VacationCalendarEvent(BaseModel):
    type: Literal['vacation_request'] = "vacation_request" # Tipo fijo
    ...
    notes: Optional[str] = None
    manager_notes: Optional[str] = None
    # Añadimos username aquí para facilitar el frontend
    username: Optional[str] = None # El frontend lo necesita
...
# 18. DocumentAttachment

# Añadir al principio de routes.py con los demás modelos
...
# 19.# === MODELOS PARA DASHBOARD STATS ===
...
# Modelo para configuración de limpieza automática
class AutoCleanupConfig(BaseModel):
    ...
    frequency: str = "daily"  # daily, every3days, weekly

AUTO_CLEANUP_CONFIG = AutoCleanupConfig()    

##21. CAMVIO CONTRASEÑAS USUARIOS #######
##########################################
...
# =============================================================================
# 22.MODELOS PYDANTIC PARA CHECKLISTS Y PROGRESO DE PASOS
# =============================================================================
...
class ChecklistProgressCreate(BaseModel):
    ...
    extra_data: Optional[Dict[str, Any]] = None  # ← CAMBIAR metadata por extra_data
...
def get_all_maintenance(
    ...
    # CONSULTA CORREGIDA Y EFICIENTE:
    # 1. Une Maintenance con WorkOrder para poder filtrar.
    # 2. Selecciona explícitamente el Plan (Maintenance) y la Orden (WorkOrder).
    query = db.query(Maintenance, WorkOrder).join(
        WorkOrder, Maintenance.generated_order_id == WorkOrder.id
    ).filter(
        WorkOrder.status == 'Cerrada'  # Filtra solo por órdenes cerradas (historial real).
    ).options(
        joinedload(Maintenance.machine) # Carga la máquina para no hacer más consultas.
    )

    # Los filtros de rol y de la interfaz se mantienen igual.
    ...
    if start and end:
        try:
            ...
            # Filtramos por la fecha de finalización de la orden de trabajo.
            query = query.filter(WorkOrder.finished_at >= sdt, WorkOrder.finished_at < edt)
        except:
            raise HTTPException(status_code=400, detail="Formato de fecha inválido. Usa YYYY-MM-DD.")
            
    # La consulta ahora devuelve tuplas (plan_de_mantenimiento, orden_de_trabajo).
    results = query.order_by(WorkOrder.finished_at.desc()).all()
    
    response_data = []
    # Iteramos sobre las tuplas.
    for plan, work_order in results:
        response_data.append({
            "id": plan.id, # ID del plan para la key de la tabla.
            "title": plan.title,
            "type": plan.type,
            "machineName": plan.machine.nombre if plan.machine else "Desconocida",
            
            # CORREGIDO: Usamos la fecha de finalización directamente del objeto work_order.
            "finished_at": work_order.finished_at.isoformat() if work_order.finished_at else None,
            
            # CORREGIDO: Usamos el ID de la orden directamente del objeto work_order.
            "work_order_id": work_order.id
        })
...
def get_ordenes(
    ...
    try:
        query = db.query(WorkOrder).options(
            ...
            joinedload(WorkOrder.repuesto),  # ⭐ SIEMPRE INCLUIR REPUESTOS
        ...
        for order in orders:
            ...
            order_data = {
                ...
                "repuesto": {
                    "id": order.repuesto.id,
                    "product_name": order.repuesto.product_name
                } if order.repuesto else None,
...
def create_orden(data: WorkOrderCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
        ...
        # Manejar inventario si se especifica
        if data.repuesto_id and data.quantity_used and data.quantity_used > 0:
            inv = db.query(Inventory).filter(Inventory.id == data.repuesto_id).first()
            if not inv:
                raise HTTPException(status_code=404, detail="Repuesto no encontrado")
            if inv.quantity < data.quantity_used:
                raise HTTPException(status_code=400, detail="Stock insuficiente")
            
            # Descontar del inventario
            inv.quantity -= data.quantity_used
            print("[DEBUG] Stock anterior:", inv.quantity + (data.quantity_used or 0))
            print("[DEBUG] Cantidad usada:", data.quantity_used or 0)
            print("[DEBUG] Nuevo stock:", inv.quantity)

        # ⭐ CREAR LA ORDEN CON VALORES FLEXIBLES
        order_dict = data.dict(exclude={'checklist_data'})  # Excluir checklist_data del modelo
        order_dict.update({
...
def delete_orden(
    orden_id: int, 
    request: Request,  # ← Añadir para audit trail
    db: Session = Depends(get_db), 
    current_user: User = Depends(get_current_user)
):
    ...
    try:
        # ✅ NUEVA LÓGICA: Verificar si está referenciada por un mantenimiento preventivo
        maintenance_referencing = db.query(Maintenance).filter(
            Maintenance.generated_order_id == orden_id
        ).first()
        
        if maintenance_referencing:
            # Limpiar la referencia antes de eliminar la orden
            logger.info(f"Limpiando referencia de mantenimiento ID {maintenance_referencing.id} a orden ID {orden_id}")
            maintenance_referencing.generated_order_id = None
            
            # Si quieres que el mantenimiento vuelva a generar una orden en la próxima fecha
            # puedes mantener la next_maintenance_date como está
            # O si quieres reprogramarlo, descomenta la siguiente línea:
            # maintenance_referencing.next_maintenance_date = datetime.utcnow() + timedelta(days=1)
        
        # Capturar datos para audit trail antes de eliminar
        order_data = {
            "order_number": order.order_number,
            "title": order.title,
            "work_type": order.work_type,
        ...
        audit_manager.log_action(
            db=db,
            action="DELETE",
            entity=order,  # SQLAlchemy aún tiene referencia antes del commit
            user=current_user,
            request=request,
            old_data=order_data,
            notes=f"Orden eliminada: {order_data['order_number']} - {order_data['title']}",
            metadata={
                "had_maintenance_reference": maintenance_referencing is not None,
                "maintenance_id": maintenance_referencing.id if maintenance_referencing else None
            }
        )
        db.commit()  # Commit del audit log
        
        success_message = "Orden eliminada correctamente"
        if maintenance_referencing:
            success_message += f". Se ha limpiado la referencia del mantenimiento preventivo ID {maintenance_referencing.id}"
...
async def update_complete_orden(
    ...
    # ⭐ VALIDACIONES ESPECIALES PARA REPUESTOS
    if data.repuesto_id is not None and data.quantity_used and data.quantity_used > 0:
        repuesto = db.query(Inventory).filter(Inventory.id == data.repuesto_id).first()
        if repuesto and repuesto.quantity < data.quantity_used:
            raise HTTPException(
                status_code=400, 
                detail=f"Stock insuficiente. Disponible: {repuesto.quantity}, Solicitado: {data.quantity_used}"
            )
...
# =============================================================================
# ENDPOINTS PARA CAMBIOS DE FORMATO Y GESTIÓN DE FORMATOS
# =============================================================================

# --- MODELOS PYDANTIC PARA FORMATOS ---
...
# --- MODELOS PARA ÓRDENES DE CAMBIO DE FORMATO ---
class FormatChangeOrderCreate(BaseModel):
    ...
    # ✅ Campos clave que deben estar aquí
    machine_id: Optional[int] = None
    affected_machines: Optional[List[int]] = [] # Acepta una lista de IDs
    setup_team: Optional[List[int]] = []       # Acepta una lista de IDs
...
class FormatChangeOrderUpdate(BaseModel):
    # Campos que se pueden actualizar durante el setup
    status: Optional[Literal["Pendiente", "En curso", "En revisión", "Cerrada"]] = None
    setup_duration: Optional[float] = Field(None, ge=0)
    production_loss_hours: Optional[float] = Field(None, ge=0)
    actual_start_time: Optional[datetime] = None
...
def add_technician_to_work_order(
    ...
    can_edit = (
        current_user.role.nombre in ["Administrador", "Jefe de Mantenimiento"] or
        (current_user.role.nombre == "Jefe de Sección" and order.section_id == current_user.section_id)
    )
    
    if not can_edit:
        raise HTTPException(status_code=403, detail="No tienes permisos para modificar esta orden")
    
    # ⭐ NUEVA LÓGICA: Verificar si ya existe una asignación (activa o inactiva)
    existing = db.query(WorkOrderTechnician).filter(
        WorkOrderTechnician.work_order_id == order_id,
    ...
    if existing:
        if existing.is_active:
            # Ya está asignado y activo
            raise HTTPException(status_code=400, detail="El técnico ya está asignado a esta orden")
        else:
            # ⭐ REACTIVAR ASIGNACIÓN EXISTENTE
            existing.is_active = True
            existing.role = technician_data.role
            existing.assigned_at = datetime.utcnow()
            existing.assigned_by_id = current_user.id
            existing.notes = technician_data.notes
            existing.hours_worked = 0.0  # Reiniciar horas
            ...
            if technician_data.role == "principal":
                existing_principal = db.query(WorkOrderTechnician).filter(
                    ...
                    WorkOrderTechnician.role == "principal",
                    WorkOrderTechnician.is_active == True,
                    WorkOrderTechnician.id != existing.id  # Excluir el actual
                ).first()
            ...
            db.refresh(existing)
            existing = db.query(WorkOrderTechnician).options(
                joinedload(WorkOrderTechnician.technician)
            ).filter(WorkOrderTechnician.id == existing.id).first()
            
            return {
                "message": "Técnico reactivado correctamente",
                "assignment": {
                    "id": existing.id,
...
# ENDPOINT: Remover técnico de orden
...
def update_technician_hours(
    ...
    # Verificar permisos - técnicos pueden actualizar sus propias horas
    can_edit = (
        current_user.role.nombre in ["Administrador", "Jefe de Mantenimiento"] or
        (current_user.role.nombre == "Jefe de Sección" and order.section_id == current_user.section_id) or
