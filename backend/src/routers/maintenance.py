        ...
        for order in orders:
            ...
            if maintenance:
                response_data.append({
                    "id": maintenance.id,
                    "title": maintenance.title or "Sin título",
                    "titulo": maintenance.title or "Sin título",
                    "type": maintenance.type or "Preventivo",
                    "tipo": maintenance.type or "Preventivo",
                    "description": maintenance.description or "",
                    "descripcion": maintenance.description or "",
                    "machineName": order.machine_obj.nombre if order.machine_obj else "Desconocida",
                    "maquina_nombre": order.machine_obj.nombre if order.machine_obj else "Desconocida",
    ...
    try:
        ...
        for m in maintenances:
            try:
                ...
                maintenance_data = {
                    "id": m.id,
                    "title": m.title or "Sin título",
                    "titulo": m.title or "Sin título",
                    "description": m.description or "",
                    "descripcion": m.description or "",
                    "type": m.type or "Preventivo",
                    "tipo": m.type or "Preventivo",
                    "maquina_id": m.machine_id,
                    "maquina_nombre": machine.nombre if machine else "Máquina no encontrada",
                    "machineName": machine.nombre if machine else "Máquina no encontrada",
