...
const NotificationCenter = () => {
  ...
  // ✅ CAMBIO PRINCIPAL: Usar sessionStorage para persistir el estado
  const [hasShownInitialNotification, setHasShownInitialNotification] = useState(() => {
    return sessionStorage.getItem('hasShownInitialNotification') === 'true';
  });
  ...
  const [lastNotificationHash, setLastNotificationHash] = useState(() => {
    return sessionStorage.getItem('lastNotificationHash') || '';
  });
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // 3. Las notificaciones han cambiado desde la última vez
      if (!hasShownInitialNotification && totalUnread > 0 && currentHash !== lastNotificationHash) {
        console.log('🔔 Mostrando notificación inicial - nuevas notificaciones detectadas');
        setHasShownInitialNotification(true);
        sessionStorage.setItem('hasShownInitialNotification', 'true');
        showInitialNotification(totalUnread);
      }
      
      // Actualizar hash solo si hay cambios
      if (currentHash !== lastNotificationHash) {
        setLastNotificationHash(currentHash);
        sessionStorage.setItem('lastNotificationHash', currentHash);
      }
  ...
  const resetNotificationState = () => {
    ...
    sessionStorage.removeItem('hasShownInitialNotification');
    sessionStorage.removeItem('lastNotificationHash');
