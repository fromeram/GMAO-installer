...
// --- NUEVO: Funciones auxiliares para la conversión de tiempo ---
const convertHoursToBestUnit = (hours) => {
  if (hours === null || hours === undefined) return { value: 24, unit: 'horas' };
  if (hours % (24 * 7) === 0) return { value: hours / (24 * 7), unit: 'semanas' };
...
const MantenimientoPreventivo = () => {
  ...
  useEffect(() => {
    fetchMaintenanceWithTaskList();
  }, []);

  const fetchMaintenanceWithTaskList = async () => {
    try {
      setLoading(true);
      const maintenanceData = await fetchWithAuth('/mantenimiento-preventivo?include_task_list_details=true');
      setMaintenances(maintenanceData);
      
      const [machinesData, usersData, rolesData] = await Promise.all([
  ...
  // --- MODIFICADO: Lógica para abrir modal de edición ---
  const handleEdit = (record) => {
    setEditingMaintenance(record);
    
    // Convertir el intervalo de notificación a la mejor unidad para el formulario
  ...
  const handleCreate = () => {
    ...
    form.resetFields();
    form.setFieldsValue({ // Establecer valores por defecto
        notification_value: 1,
        notification_unit: 'dias',
    });
    setAssignmentType('user');
    setModalVisible(true);
  };

  // --- MODIFICADO: Lógica para guardar con conversión ---
  ...
  const handleSubmit = async (payload) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        message.error(response.detail || 'Error al crear');
      }
    } catch (error) {
  ...
  const handleUpdate = async (payload) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        message.error(response.detail || 'Error al actualizar');
      }
    } catch (error) {
      message.error(error.message || 'Error al actualizar el mantenimiento');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    try {
      ...
      if (response.success) {
        ...
        Modal.success({
          ...
          content: (
            <div>
              <p><strong>Número:</strong> {response.work_order.order_number}</p>
        ...
        message.error(response.detail || 'Error al generar orden');
      }
    } catch (error) {
      message.error(error.message || 'Error al generar orden');
    }
  };
      ...
      {canCreateOrEdit && (
        ...
        >
          <Form form={form} layout="vertical" onFinish={handleSave}>
            ...
            {/* --- MODIFICADO: Campo de Antelación de Notificación --- */}
