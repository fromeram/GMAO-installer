// src/pages/InventoryReport.js (Con Fórmulas Excel)
...
const InventoryReport = () => {
  ...
  const isAuthorized = useMemo(() => {
    ...
    return ['Administrador', 'Contabilidad', 'Calidad', 'Jefe de Mantenimiento'].includes(userRole);
  }, [currentUser]);
  ...
  // --- FUNCIÓN DE EXPORTACIÓN CON FÓRMULAS ---
  const handleExport = () => {
    ...
    message.loading({ content: 'Generando Excel con fórmulas...', key: 'export' });
    ...
    selectedData.forEach(item => {
      ...
      const exportItem = {
        ...
        // NO calculamos aquí - se hará con fórmula
        'Valor Total Línea (€)': 0  // Placeholder
      };
    ...
    try {
      ...
      Object.keys(dataByType).forEach(tipo => {
        if (dataByType[tipo].length > 0) {
          ...
          for (let R = range.s.r + 1; R <= range.e.r; ++R) {
            const cellD = XLSX.utils.encode_cell({r: R, c: 3}); // Cantidad
            const cellG = XLSX.utils.encode_cell({r: R, c: 6}); // Precio
            const cellH = XLSX.utils.encode_cell({r: R, c: 7}); // Descuento
            const cellI = XLSX.utils.encode_cell({r: R, c: 8}); // Valor Total
      ...
      sheetSummaryData.forEach((data, index) => {
        const rowIndex = index + 4; // Empezamos en fila 4 (índice 3)
        summaryData.push([
          data.tipo,
          data.count,
      ...
      // Agregar fórmulas a la hoja resumen
      sheetSummaryData.forEach((data, index) => {
        const rowIndex = index + 3; // Filas de datos empiezan en índice 3
        const sheetName = data.sheetName;
        
        // Fórmula para cantidad total: suma la columna D de la hoja correspondiente
        summarySheet[XLSX.utils.encode_cell({r: rowIndex, c: 2})] = {
          f: `=SUM(${sheetName}.D:D)`
        };
        
        // Fórmula para valor total: suma la columna I de la hoja correspondiente
      ...
      summarySheet[XLSX.utils.encode_cell({r: totalGeneralRow, c: 2})] = {
        f: `=SUM(C4:C${3 + sheetSummaryData.length})`
      };
      summarySheet[XLSX.utils.encode_cell({r: totalGeneralRow, c: 3})] = {
        f: `=SUM(D4:D${3 + sheetSummaryData.length})`,
        z: '#,##0.00€'
      };
      ...
      const filename = selectedType === 'todos' 
        ? 'reporte_inventario_todos_tipos_con_formulas.xlsx'
        : `reporte_inventario_${selectedType}_con_formulas.xlsx`;
        
      XLSX.writeFile(workbook, filename);
      message.success({ content: 'Excel con fórmulas generado con éxito!', key: 'export', duration: 3 });
    } catch (exportError) {
    ...
    <div style={{ padding: '24px' }}>
      ...
      <Card>
        <Space style={{ marginBottom: 16 }}>
          ...
          >
            Exportar Selección a Excel (Con Fórmulas)
          </Button>
          <Text type="secondary">
