...
import { DeleteOutlined, EditOutlined, PlusOutlined, SearchOutlined, FileExcelOutlined, EyeOutlined, ToolOutlined, CheckSquareOutlined, SettingOutlined, UserOutlined } from '@ant-design/icons';import { Link, useNavigate, useLocation } from 'react-router-dom';
...
const Ordenes = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      return ordersWithKeys; // ✅ AÑADIR ESTA LÍNEA
    } catch (err) {
  ...
  const initializeData = async () => {
    ...
    // Después de cargar los datos, verificar si hay que abrir un modal
    if (location.state?.openOrderId) {
      const orderIdToOpen = location.state.openOrderId;
      
      // Buscar la orden en workOrders (que ya está actualizado por loadData)
      // Usar una función que espere a que workOrders se actualice
      setTimeout(() => {
        setWorkOrders(currentOrders => {
          const orderToOpen = currentOrders.find(o => o.id === orderIdToOpen);
          
          if (orderToOpen) {
            showOrderDetail(orderToOpen);
          } else {
            // Si no se encuentra, intentar cargar directamente por ID
            showOrderDetail({ id: orderIdToOpen }).catch(() => {
              message.error(`No se pudo encontrar la orden con ID ${orderIdToOpen}.`);
            });
          }
          
          return currentOrders; // No cambiar el estado
        });
  ...
  const handleEditOrder = (order) => {
    setDetailModalVisible(false); // Cierra el modal de detalle si estaba abierto
  ...
  const columns = [
    ...
    { 
      title: 'Máquina / Equipo', // Título actualizado
      dataIndex: 'machine', 
      key: 'machine', 
      width: 180, // Un poco más de espacio
      render: (machine, record) => {
        // Si es un cambio de formato y tiene varias máquinas
        if (record.is_format_change && record.affected_machines && record.affected_machines.length > 0) {
          const machineNames = record.affected_machines
            .map(id => machines.find(m => m.id === id)?.nombre || `ID ${id}`)
            .join(', ');
          
          return (
            <Tooltip title={machineNames}>
              <Tag color="purple">{record.affected_machines.length} Máquinas</Tag>
        ...
        // Lógica original para una sola máquina
    ...
    },
    {
      ...
      width: 150,
      render: (_, record) => {
        // Primero, revisa el nuevo sistema de múltiples técnicos
        const activeTechs = (record.technicians || []).filter(t => t.is_active !== false);
        ...
        if (activeTechs.length > 0) {
          return (
            <Space direction="vertical" size={2}>
              {activeTechs.map(t => (
                <Tag 
                  color={t.role === 'principal' ? 'blue' : 'default'} 
                  key={t.user_id}
                  icon={<UserOutlined />}
                >
                  {t.username}
        ...
        // Si no hay técnicos en la lista nueva, busca en el sistema antiguo
        if (record.assigned_to) {
          return <Tag color="blue" icon={<UserOutlined />}>{record.assigned_to.username}</Tag>;
        }

        // Si no hay ninguno en ninguno de los dos sistemas
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      >
        <FormatChangeForm
          sections={sections}      // ✅ Pasar sections
          users={users}           // ✅ Pasar users  
          machines={machines}     // ✅ Pasar machines
