    ...
    <div style={{ padding: '20px' }}>
      ...
      {detailModalVisible && selectedOrderDetail && (
        <WorkOrderDetailModal
          ...
          onEdit={canEditOrder(selectedOrderDetail) ? (order) => {
            setDetailModalVisible(false);
            setEditingOrder(order);
            setModalVisible(true);
          } : null}
          showEditButton={canEditOrder(selectedOrderDetail)}
          onUpdate={handleTechnicianUpdate} // ⭐ CALLBACK PARA ACTUALIZAR
          loading={detailLoading}
        />
      )}

      {/* ⭐ Modal para checklist RESTAURADO */}
      <Modal
        ...
        width={800}
      >
        {orderTaskList && selectedOrderForChecklist && (
          <InteractiveTaskListChecklist
            ...
            onComplete={() => {
              Modal.confirm({
                ...
                content: '¿Deseas marcar esta orden de trabajo como "En revisión" o "Cerrada"?',
                okText: 'En revisión',
                cancelText: 'Cerrada',
                onOk: async () => {
                  await handleStatusChange(selectedOrderForChecklist.id, 'En revisión');
                  setChecklistModalVisible(false);
                  message.success('Checklist completado y orden marcada como "En revisión"');
                },
                onCancel: async () => {
                  await handleStatusChange(selectedOrderForChecklist.id, 'Cerrada');
                  setChecklistModalVisible(false);
                  message.success('Checklist completado y orden cerrada');
                }
              });
            }}
          />
